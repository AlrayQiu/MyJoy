...
HID_REPORT_DESCRIPTOR G_DefaultReportDescriptor[] = {
    0x06,
    0x00,
    0xFF, // USAGE_PAGE (Vender Defined Usage Page)
    0x09,
    0x01, // USAGE (Vendor Usage 0x01)
    0xA1,
    0x01, // COLLECTION (Application)
    0x85,
    CONTROL_FEATURE_REPORT_ID, // REPORT_ID (1)
    0x09,
    0x01, // USAGE (Vendor Usage 0x01)
    0x15,
    ...
    0x75,
    0x08, // REPORT_SIZE (0x08)
    0x96,
    (FEATURE_REPORT_SIZE_CB & 0xff),
    (FEATURE_REPORT_SIZE_CB >> 8), // REPORT_COUNT
    0xB1,
    0x00, // FEATURE (Data,Ary,Abs)
    0x09,
    0x01, // USAGE (Vendor Usage 0x01)
    0x75,
    0x08, // REPORT_SIZE (0x08)
    0x96,
    (INPUT_REPORT_SIZE_CB & 0xff),
    (INPUT_REPORT_SIZE_CB >> 8), // REPORT_COUNT
    0x81,
    ...
    0x08, // REPORT_SIZE (0x08)
    0x96,
    (OUTPUT_REPORT_SIZE_CB & 0xff),
    (OUTPUT_REPORT_SIZE_CB >> 8), // REPORT_COUNT
    0x91,
...
Arguments:
    ...
    IoControlCode - The driver or system defined IOCTL associated with the
request
...
--*/
{
  ...
  case IOCTL_HID_GET_DEVICE_ATTRIBUTES: // METHOD_NEITHER
    ...
    status = RequestCopyFromBuffer(
        Request, &queueContext->DeviceContext->HidDeviceAttributes,
  ...
  case IOCTL_HID_GET_REPORT_DESCRIPTOR: // METHOD_NEITHER
    ...
    //
    status = RequestCopyFromBuffer(
        Request, deviceContext->ReportDescriptor,
  ...
  case IOCTL_HID_SET_OUTPUT_REPORT: // METHOD_IN_DIRECT
    ...
    //
    // HID minidriver IOCTL uses HID_XFER_PACKET which contains an embedded
    // pointer.
    //
    ...
    //
    // UMDF cannot handle embedded pointers when marshalling buffers between
    // processes. Therefore a special driver mshidumdf.sys is introduced to
    // convert such IRPs to new IRPs (with new IOCTL name like
    // IOCTL_UMDF_HID_Xxxx) where:
    //
    //   reportBuffer - passed as one buffer inside the IRP
    //   reportId     - passed as a second buffer inside the IRP
    //
    // The new IRP is then passed to UMDF host and driver for further
    // processing.
    //
  ...
  case IOCTL_HID_SEND_IDLE_NOTIFICATION_REQUEST: // METHOD_NEITHER
                                                 //
                                                 // This has the USBSS Idle
                                                 // notification callback. If
                                                 // the lower driver can handle
                                                 // it (e.g. USB stack can
                                                 // handle it) then pass it down
                                                 // otherwise complete it here
                                                 // as not inplemented. For a
                                                 // virtual device, idling is
                                                 // not needed.
                                                 //
                                                 // Not implemented. fall
                                                 // through...
                                                 //
  ...
  case IOCTL_GET_PHYSICAL_DESCRIPTOR:            // METHOD_OUT_DIRECT
                                                 //
                                      // We don't do anything for these IOCTLs
                                      // but some minidrivers might.
                                      //
...
RequestCopyFromBuffer(
    _In_ WDFREQUEST Request, _In_ PVOID SourceBuffer,
...
NTSTATUS
ReadReport(_In_ PQUEUE_CONTEXT QueueContext, _In_ WDFREQUEST Request,
...
--*/
{
  ...
  //
  myAttributes =
      (PMY_DEVICE_ATTRIBUTES)(packet.reportBuffer + sizeof(packet.reportId));
...
--*/
{
  ...
  default:
    status = STATUS_NOT_IMPLEMENTED;
    KdPrint(
        ("SetFeature: Unknown control Code 0x%x\n", controlInfo->ControlCode));
    break;
  }
...
Routine Description:
    ...
    Helper routine to decode IOCTL_HID_GET_INDEXED_STRING and
...
--*/
{
  ...
  inputValue =
      PtrToUlong(requestParameters.Parameters.DeviceIoControl.Type3InputBuffer);
...
--*/
{
  ...
  if (NT_SUCCESS(status)) {
    ...
    status = RequestCopyFromBuffer(Request, VHIDMINI_DEVICE_STRING,
...
Routine Description:
    ...
    - The request reaches the driver's default queue. As data may not be
avaiable yet, the request is forwarded to a second manual queue temporarily.
...
--*/
{
  ...
  WDF_TIMER_CONFIG_INIT_PERIODIC(&timerConfig, EvtTimerFunc,
...
--*/
{
  ...
  if (NT_SUCCESS(status)) {
    ...
    status = WdfRegistryQueryMemory(hKey, &valueName, NonPagedPool, &attributes,
                                    &memory, NULL);
